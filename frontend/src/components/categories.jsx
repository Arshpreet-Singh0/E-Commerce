      ...
      >
        ...
        <div className="container relative flex overflow-auto scroll-snap-x-mandatory">
          {category.products.map((product) => (
            <div
              key={product._id}
              className="box justify-center align-middle items-center flex-shrink-0 w-full sm:w-1/2 md:w-full lg:w-1/4 h-auto bg-white rounded-lg shadow-md scroll-snap-start p-4"
              onClick={() => handleProductClick(product._id)} // Click event to navigate
            >
              <img
                src={product.images[0]?.url || '/path/to/fallback-image.jpg'}
                alt={product.name || 'Product'}
                className="w-auto h-56 object-cover rounded-lg mb-2"
              />
              <h2 className="text-lg font-semibold">{product.name}</h2>
              <p className="text-gray-600">${product.price}</p>
              <button className="bg-black text-white px-4 py-2 hover:bg-green-600 mt-2">
                Shop Now
              </button>
            </div>
          ))}
